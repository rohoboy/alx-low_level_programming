...
/**
 * print_magic - A functon that prints the magic numbers of an ELF header.
 * @e_ident: A pointer to an array containing the ELF magic numbers.
 *Return: void
 */
void print_magic(unsigned char *e_ident)
...
/**
 * print_entry - A functon that prints the entry point of an ELF header.
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	if (r == -1)
	{
		free(header);
		close_elf(o);
		dprintf(STDERR_FILENO, "Error: `%s`: Unable to locate such file\n", argv[1]);
		exit(98);
	}
